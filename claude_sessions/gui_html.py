...
<title>claudectl</title>
<style>
:root{
  --bg:#0d1117; --bg2:#111827; --panel:#161e2e; --panel2:#1b2436;
  --line:#232f47; --txt:#dbe4f3; --dim:#7d8aa5; --dim2:#55627c;
  --cyan:#7dcfff; --violet:#8a5cf6; --ok:#3fdd9d; --warn:#f7b955; --err:#ff6b81;
  --code:#0a0e16; --onacc:#0d1117;
  --grad:linear-gradient(135deg,#7dcfff,#8a5cf6);
  --r:12px; --shadow:0 8px 28px rgba(0,0,0,.45);
}
...
.logo{width:34px;height:34px;border-radius:9px;background:var(--grad);
  display:grid;place-items:center;font-weight:800;font-size:17px;color:var(--onacc)}
...
.search input{width:100%;background:var(--panel);border:1px solid var(--line);
  border-radius:8px;padding:7px 12px;color:var(--txt);outline:none;font-size:13px}
.search input:focus{border-color:var(--cyan)}
.plist{flex:1;overflow-y:auto;padding:2px 8px 8px;min-height:80px}
.proj{padding:8px 11px;border-radius:9px;cursor:pointer;margin-bottom:1px;
  border:1px solid transparent;transition:background .12s}
.proj:hover{background:var(--panel)}
.proj.sel{background:var(--panel2);border-color:var(--line)}
.proj .nm{font-weight:600;font-size:13px;display:flex;align-items:center;gap:7px}
.proj .pt{color:var(--dim2);font-size:11px;white-space:nowrap;overflow:hidden;
  text-overflow:ellipsis}
.tag{font-size:10px;padding:1px 7px;border-radius:20px;background:#8a5cf622;
  color:#b39bf7;border:1px solid #8a5cf644;font-weight:500}
.tag.warn{background:#f7b95522;color:var(--warn);border-color:#f7b95544}
.tag.ok{background:#3fdd9d22;color:var(--ok);border-color:#3fdd9d44}
.nav{border-top:1px solid var(--line);padding:8px}
.nav .it{display:flex;align-items:center;gap:9px;padding:7px 12px;
  border-radius:8px;cursor:pointer;color:var(--dim);font-size:13px}
.nav .it:hover{background:var(--panel);color:var(--txt)}
.nav .it.sel{background:var(--panel2);color:var(--txt)}
...
.seg button{background:transparent;border:0;color:var(--dim);padding:4px 12px;
  cursor:pointer;font-size:12px}
.seg button.on{background:var(--grad);color:var(--onacc);font-weight:700}

/* ── main ── */
.main{display:flex;flex-direction:column;min-width:0;min-height:0}
.top{display:flex;align-items:center;gap:12px;padding:13px 26px;
...
.btn.pri{background:var(--grad);color:var(--onacc);border:0}
.btn.pri:hover{filter:brightness(1.1)}
.btn.danger:hover{border-color:var(--err)}
.btn.sm{padding:3px 10px;font-size:12px;border-radius:7px}
.ubar{display:flex;flex-direction:column;gap:2px;padding:6px 26px;
  background:var(--bg2);border-bottom:1px solid var(--line);font-size:11.5px}
.ubar:empty{display:none}
.urow{display:flex;align-items:center;gap:14px;flex-wrap:wrap}
.urow .uacct{color:var(--dim);min-width:180px;white-space:nowrap;
  overflow:hidden;text-overflow:ellipsis;font-variant-numeric:tabular-nums}
.uwin{display:flex;align-items:center;gap:6px;white-space:nowrap}
.uwin .ulbl{color:var(--dim2);text-transform:capitalize}
.uwin .ubarm{width:64px;height:6px;background:#232f47;border-radius:4px;overflow:hidden}
.uwin .ubarm i{display:block;height:100%;background:var(--grad)}
.uwin.hot .ubarm i{background:linear-gradient(90deg,#f7b955,#ff6b81)}
.uwin .upct{font-variant-numeric:tabular-nums;min-width:34px;text-align:right}
.uwin.hot .upct{color:var(--err)}
.uwin .urst{color:var(--dim2)}
.tabs{display:flex;gap:2px;padding:0 26px;background:var(--bg2);
  border-bottom:1px solid var(--line)}
.tab{padding:9px 16px;cursor:pointer;color:var(--dim);font-size:13px;
  border-bottom:2px solid transparent;font-weight:600}
.tab:hover{color:var(--txt)}
.tab.sel{color:var(--cyan);border-bottom-color:var(--cyan)}
.content{flex:1;overflow-y:auto;padding:20px 26px;min-height:0}

/* home */
.hero{max-width:760px;margin:5vh auto 0;text-align:center}
.hero .big{width:74px;height:74px;border-radius:20px;background:var(--grad);
  margin:0 auto 18px;display:grid;place-items:center;font-weight:800;
  font-size:36px;color:var(--onacc);box-shadow:var(--shadow)}
.hero h2{font-size:22px;margin-bottom:6px}
.hero p{color:var(--dim);margin-bottom:24px}
.qr{display:flex;flex-direction:column;gap:8px;max-width:560px;margin:0 auto;text-align:left}
.lbl{color:var(--dim);font-size:11px;text-transform:uppercase;
  letter-spacing:1.2px;margin-bottom:4px}
.qcard{display:flex;align-items:center;gap:12px;background:var(--panel);
  border:1px solid var(--line);border-radius:var(--r);padding:11px 16px;
  cursor:pointer;transition:border-color .12s}
.qcard:hover{border-color:var(--cyan)}
.qcard .star{color:var(--warn);font-size:16px}
.qcard .info{flex:1;min-width:0}
.qcard .t{font-weight:600;font-size:13.5px;white-space:nowrap;
  overflow:hidden;text-overflow:ellipsis}
.qcard .s{color:var(--dim);font-size:12px}

/* lists & cards */
.slist{display:flex;flex-direction:column;gap:8px;max-width:940px}
.sess{display:flex;align-items:center;gap:13px;background:var(--panel);
  border:1px solid var(--line);border-radius:var(--r);padding:11px 16px;
  transition:border-color .12s}
.sess:hover{border-color:#3a4c72}
.sess .dot{width:9px;height:9px;border-radius:50%;background:var(--ok);flex:none}
.sess .info{flex:1;min-width:0}
.sess .t{font-weight:600;font-size:13.5px;white-space:nowrap;overflow:hidden;
  text-overflow:ellipsis}
.sess .t input{background:var(--bg);border:1px solid var(--cyan);color:var(--txt);
  border-radius:6px;padding:2px 8px;font-size:13px;width:60%}
.sess .meta{color:var(--dim);font-size:12px;display:flex;gap:11px;margin-top:1px;flex-wrap:wrap}
.sess .acts{display:flex;gap:6px;opacity:0;transition:opacity .12s;flex-wrap:wrap;justify-content:flex-end}
.sess:hover .acts{opacity:1}
.empty{color:var(--dim);text-align:center;margin-top:7vh}
.card{background:var(--panel);border:1px solid var(--line);border-radius:var(--r);
  padding:16px 18px;margin-bottom:12px;max-width:940px}
...
.tbl th{color:var(--dim);text-align:left;font-weight:600;font-size:11px;
  text-transform:uppercase;letter-spacing:.7px;padding:5px 8px;
  border-bottom:1px solid var(--line)}
.tbl td{padding:6px 8px;border-bottom:1px solid var(--panel2)}
.tbl tr:hover td{background:var(--panel2)}
.tbl .num{text-align:right;font-variant-numeric:tabular-nums}
.bar{height:8px;background:#232f47;border-radius:6px;overflow:hidden;min-width:80px}
.bar i{display:block;height:100%;background:var(--grad);border-radius:6px}
.bar.hot i{background:linear-gradient(90deg,#f7b955,#ff6b81)}

/* modal / drawer / toast */
/* no backdrop-filter — blur repaints flicker under QtWebEngine */
.ovl{position:fixed;inset:0;background:#05070ecc;display:none;
  align-items:center;justify-content:center;z-index:50}
.ovl.show{display:flex}
.modal{background:var(--panel);border:1px solid var(--line);border-radius:16px;
  padding:24px 26px;width:540px;max-width:94vw;max-height:88vh;overflow-y:auto;
...
.fld label{display:block;color:var(--dim);font-size:11px;text-transform:uppercase;
  letter-spacing:.8px;margin-bottom:4px}
.fld select,.fld input,.fld textarea{width:100%;background:var(--bg);
...
.drawer.show{right:0}
.drawer .dhead{display:flex;align-items:center;gap:10px;padding:15px 20px;
  border-bottom:1px solid var(--line)}
.drawer .dhead h3{flex:1;font-size:14px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.drawer .dbody{flex:1;overflow-y:auto;padding:16px 20px}
.msg{margin-bottom:14px}
.msg .who{font-size:11px;text-transform:uppercase;letter-spacing:1px;
  color:var(--cyan);margin-bottom:3px;font-weight:700}
.msg.user .who{color:var(--violet)}
.msg .body{background:var(--panel);border:1px solid var(--line);
  border-radius:10px;padding:10px 13px;font-size:13px;white-space:pre-wrap;
  word-break:break-word}
.diff{background:var(--code);border:1px solid var(--line);border-radius:10px;
  padding:10px 0;font:12px/1.5 Consolas,monospace;overflow-x:auto;
  max-height:52vh;overflow-y:auto}
.diff div{padding:0 14px;white-space:pre}
.diff .a{background:#12351f;color:#7ee2a8}
.diff .d{background:#3b1620;color:#ff9daa}
.diff .h{color:var(--cyan)}
.toast{position:fixed;bottom:24px;left:50%;transform:translateX(-50%) translateY(80px);
  background:var(--panel2);border:1px solid var(--line);border-radius:10px;
  padding:11px 20px;font-size:13px;box-shadow:var(--shadow);opacity:0;
  transition:all .25s;z-index:60;pointer-events:none;max-width:80vw}
.toast.show{opacity:1;transform:translateX(-50%) translateY(0)}
.toast.ok{border-color:#3fdd9d66}
.toast.err{border-color:#ff6b8166}
...
.msgs{font-size:12px;color:var(--dim);margin-top:10px;max-height:120px;overflow-y:auto}
.msgs div{padding:2px 0}
.msgs .bad{color:var(--err)}
...
.chip.on{background:var(--grad);color:var(--onacc);border:0;font-weight:700}
.ic{width:15px;height:15px;vertical-align:-3px;fill:currentColor;flex:none}
.nav .ic,.btn .ic{margin-right:2px}
.chips{display:flex;flex-wrap:wrap;gap:4px}
.lrow{display:flex;gap:6px;margin-bottom:6px}
.lrow input{flex:1;background:var(--bg);border:1px solid var(--line);
  border-radius:8px;padding:7px 10px;color:var(--txt);font-size:13px;outline:none}
...
<div class="app">
  <aside class="side">
    ...
    <div class="foot">
      <span>interface</span>
      <div class="seg">
        <button id="segTui">TUI</button>
        <button id="segGui">GUI</button>
      </div>
    </div>
  </aside>
  ...
  <main class="main">
    <div class="top" id="topbar">
      ...
      <span id="pactions" style="display:none">
        ...
        <button class="btn" id="bCont">Continue</button>
        <button class="btn pri" id="bNew">New session</button>
      </span>
    </div>
...
<div class="ovl" id="ovl">
  <div class="modal">
    <h3 id="mTitle">New session</h3>
    <div class="sub" id="mSub"></div>
    <div>
      <div class="fld"><label>Effort</label><div class="chips" id="fEffort"></div></div>
      <div class="fld"><label>Model</label><div class="chips" id="fModel"></div></div>
      <div class="fld"><label>Permission mode</label><div class="chips" id="fPerm"></div></div>
      <div class="fld" id="fAcctWrap"><label>Account</label><div class="chips" id="fAcct"></div></div>
      <div class="fld"><label>Thinking cap</label><div class="chips" id="fThink"></div></div>
      <div class="fld"><label>Subagent model</label><div class="chips" id="fSub"></div></div>
      <div class="fld" id="fWtWrap"><label>Worktree</label><div class="chips" id="fWt"></div></div>
      <div class="fld" id="fNameWrap"><label>Session name</label>
        <input id="fName" placeholder="optional"></div>
    </div>
    <div class="mrow">
...
<!-- job progress + gate modal -->
<div class="ovl" id="jovl">
  <div class="modal" id="jmodal">
    <h3 id="jTitle"><span class="spin"></span> <span id="jLabel">Working…</span></h3>
    <div class="sub" id="jSub"></div>
    <div id="jGate" style="display:none">
      <div class="diff" id="jDiff"></div>
      <div class="mrow">
        <button class="btn" id="jReject">Discard</button>
        <button class="btn pri" id="jApply">Apply</button>
      </div>
    </div>
    <div class="msgs" id="jMsgs"></div>
    <div class="mrow" id="jCancelRow">
...
<div class="ovl" id="povl">
  <div class="modal">
    ...
    <div class="mrow">
      ...
      <button class="btn pri" id="pOk">OK</button>
    </div>
  </div>
</div>
  ...
  const poll=async()=>{
    ...
    if(st.status==='awaiting'&&st.gate){
      ...
      $('#jApply').onclick=async()=>{await post(`/api/job/${jid}/decide`,{apply:true});
        ...
        $('#jTitle').innerHTML='<span class="spin"></span> <span id="jLabel">Applying…</span>';
        setTimeout(poll,300);};
    ...
    else toast(st.error||'Failed','err');
  };
...
const NAV=[['usage','chart','Usage'],['searchp','search','Search'],['mcp','plug','MCP servers'],
  ['agents','robot','Agents'],['hooks','link','Hooks'],['accounts','group','Accounts'],
...
function drawHome(){
  ...
  const qr=ST.recent.map((r,i)=>`
    <div class="qcard" onclick="quickResume(${i})">
      <span class="star">${ic('star')}</span>
      <div class="info"><div class="t">${esc(r.name)}</div>
        <div class="s">${esc(r.project)}</div></div>
      <button class="btn sm">Resume</button>
    </div>`).join('');
  $('#content').innerHTML=`<div class="hero">
    <div class="big">C</div>
    <h2>Your Claude Code workspace</h2>
    <p>Pick a project on the left, or jump back in below.</p>
    <div class="qr">${qr?'<div class="lbl">Quick resume</div>'+qr:''}</div></div>`;
}
function quickResume(i){
  const r=ST.recent[i];
  askLaunch({title:'Resume — '+r.name,sub:r.project,isNew:false,
    path:r.path,enc:r.encoded,choice:'resume:'+r.sid,cfgdir:r.cfgdir});
}
...
function drawProject(){
  ...
  ({sessions:drawSessions,memory:drawMemory,claudemd:drawClaudeMd,
    audit:drawAudit,pusage:drawProjUsage,tools:drawTools}[TAB])();
}
  ...
  $('#content').innerHTML=hdr+'<div class="slist">'+SESS.map((s,i)=>{
    ...
    return `<div class="sess">
      ...
      <div class="acts">${archived?`
        <button class="btn sm" onclick="restoreS(${i})">Restore</button>
        <button class="btn sm danger" onclick="deleteS(${i},true)">Delete</button>`:`
        ...
        <button class="btn sm pri" onclick="resumeS(${i})">Resume</button>`}
      </div></div>`;}).join('')+'</div>';
}
function resumeS(i){const s=SESS[i];
  askLaunch({title:'Resume — '+(s.title||s.sid.slice(0,8)),sub:CUR.name,isNew:false,
    path:CUR.path,enc:CUR.encoded,choice:'resume:'+s.sid,cfgdir:s.cfgdir});}
...
async function drawMemory(){
  $('#content').innerHTML='<div class="empty"><span class="spin"></span> Loading…</div>';
  const c=C();
  const [st,les,ws]=await Promise.all([
    api('/api/memory/state?'+qs(c)),api('/api/lessons?'+qs(c)),
    api('/api/workspace-status?'+qs(c))]);
...
function drawTools(){
  $('#content').innerHTML=`
    ...
    <div class="card"><h3>${ic('map')} Plan → Execute</h3>
      <p style="color:var(--dim);font-size:13px;margin-bottom:10px">Write the plan with one model (default: ${esc(ST.plan_model||'opus')}), execute with another.</p>
      <button class="btn" onclick="planFlow()">Describe task…</button></div>
...
async function planFlow(){
  const v=await ask('Plan → Execute',[{label:'Task description',type:'textarea'}]);
  if(v===null||!v[0].trim())return;
  runJob('plan_make',{path:CUR.path,task:v[0]},async st=>{
    if(!st.result){toast('Plan came back empty','err');return;}
    const fs=CUR.path.replace(/\\/g,'/')+'/.claudectl/plan.md';
    const ok=await confirmBox('Plan ready','Launch the execute session in a new terminal?');
    if(ok){const r=await post('/api/plan/launch',{path:CUR.path,plan_file:st.result});
      toast(r.ok?'Execute session launched':'Failed','ok');}
  });
}
...
async function drawPage(id){
  $('#ttl').textContent=({usage:'Usage',searchp:'Search all sessions',mcp:'MCP servers',
    agents:'Agents',hooks:'Hooks',accounts:'Accounts',settings:'Settings',helpp:'Help'})[id]||id;
  $('#tpath').textContent='';
  $('#content').innerHTML='<div class="empty"><span class="spin"></span> Loading…</div>';
  await ({usage:pgUsage,searchp:pgSearch,mcp:pgMcp,agents:pgAgents,
...
async function pgSettings(){
  const o=ST.options;
  const fld=(id,label)=>`<div class="fld"><label>${label}</label><div class="chips" id="${id}"></div></div>`;
  ...
  chipsFill($('#sShell'),['auto','qt','edge','browser'],
    ...
    ST.gui_shell||'auto');
  chipsFill($('#sTheme'),Object.keys(ST.themes||{}),null,ST.theme||'default');
...
async function setSave(){
  ...
  ST=await api('/api/state');applyTheme(ST.theme);toast('Settings saved','ok');
}
...
function askLaunch(cfg){
  ...
  chipsFill($('#fEffort'),o.efforts,null,d.effort);
  chipsFill($('#fModel'),o.models,o.model_labels,d.model);
...
async function doLaunch(){
  ...
  const opts={effort:chipVal($('#fEffort')),model:chipVal($('#fModel')),
